...
```{r preliminary_clean_up_predictors, echo=F}
...
#fin.preds_num.be <- cbind((1-expo.scale(fin.preds_num))/2,(1+expo.scale(fin.preds_num))/2)

fin.preds_num.be <- escofier.transform(fin.preds_num)

#colnames(fin.preds_num.be) <- c(paste0(colnames(fin.preds_num),"-"),paste0(colnames(fin.preds_num),"+"))
...
```{r find_outliers_predictors, echo=F}

ca.res <- epCA(fin.dat,graphs=F)
chi.mahal <- cbind(rowSums(ca.res$ExPosition.Data$fi^2),rowSums(ca.res$ExPosition.Data$pdq$p^2))

  ## need to find outliers here...
#plot(cbind(rowSums(ca.res$ExPosition.Data$fi^2),rowSums(ca.res$ExPosition.Data$pdq$p^2)))


#class.order
#unique(class.order)
# pih <- pointsInHull(chi.mahal,chi.mahal,percentage=.75)
# pih.cols <- ifelse(pih==1,"blue","red")
# plot(cbind(rowSums(ca.res$ExPosition.Data$fi^2),rowSums(ca.res$ExPosition.Data$pdq$p^2)),col=pih.cols,pch=20)
# 
# these.cols <- rep("blue",nrow(chi.mahal))
# these.cols[which(chi.mahal[,1] > 3 | chi.mahal[,2] > 250)] <- "red"
# plot(cbind(rowSums(ca.res$ExPosition.Data$fi^2),rowSums(ca.res$ExPosition.Data$pdq$p^2)),col=these.cols,pch=20)
...
#fin.preds_num.inliners.be <- cbind((1-expo.scale(fin.preds_num.inliers))/2,(1+expo.scale(fin.preds_num.inliers))/2)

#colnames(fin.preds_num.inliners.be) <- c(paste0(colnames(fin.preds_num.inliers),"-"),paste0(colnames(fin.preds_num.inliers),"+"))
...
ca.res.inliers <- epCA(fin.dat.inliers,graphs=F)
chi.mahal.inliers <- cbind(rowSums(ca.res.inliers$ExPosition.Data$fi^2),rowSums(ca.res.inliers$ExPosition.Data$pdq$p^2))

#plot(cbind(rowSums(ca.res.inliers$ExPosition.Data$fi^2),rowSums(ca.res.inliers$ExPosition.Data$pdq$p^2)))
dim(fin.dat.inliers)

```

```{r preliminary_clean_up_outcomes, echo=F}
...
#fin.outs_num.be <- cbind((1-expo.scale(fin.outs_num))/2,(1+expo.scale(fin.outs_num))/2)
#colnames(fin.outs_num.be) <- c(paste0(colnames(fin.outs_num),"-"),paste0(colnames(fin.outs_num),"+"))
fin.outs_num.be <- mean.impute(fin.outs_num.be)
fin.dat.outs <- cbind(fin.outs_num.be,makeNominalData(fin.outs_cat))

#dim(fin.dat.outs)
#summary(fin.dat.outs)
#ca.res.outs <- epCA(fin.dat.outs,graphs=F)

```

```{r out_as_supp}
sup.cols <- supplementaryCols(fin.dat.outs,res = ca.res.inliers)
a<-prettyPlot(sup.cols$fjj,dev.new=F)
```

```{r plsca, echo=F}
plsca.res <- tepPLSCA(DATA1 = fin.dat.inliers,DATA2 = fin.dat.outs,make_data1_nominal = F,make_data2_nominal = F,graphs=F, DESIGN = fin.des[rownames(fin.dat.inliers),"DEMENTIA"], make_design_nominal = T)
  ## type doesn't seem to be a big deal, but inst vs. community is for both LVs, but more so on LV2 than 1.

## there are clearly 3 LVs that need interpretation
## I think 3 is a big deal -- there is a caregiver education component.

## need some resamplin' here
boot.iters <- 100
...
FIN.DX.DES <- makeNominalData(as.matrix(fin.des[rownames(fin.dat.inliers),"DEMENTIA"]))	
for(i in 1:boot.iters){
  boot.sample <- boot.samples(fin.dat.inliers,FIN.DX.DES,T)
  DAT1.boot <- fin.dat.inliers[boot.sample,]
  DAT2.boot <- fin.dat.outs[boot.sample,]  
...
fi.bsrs <- boot.ratio.test(fi.boot,critical.value = 3.5)
fj.bsrs <- boot.ratio.test(fj.boot,critical.value = 3.5)	## stability of each point; but because of dual coding this is incorrect.


### I have to pick up here and clean up the plotting code.
  ## I should make a function to make this easier
...
fi.2and2 <- which(fi.bsrs$sig.boot.ratios[,2]==1 & fi.bsrs$sig.boot.ratios[,3]==1)
...
points(plsca.res$TExPosition.Data$fi[fi.2and2,c(2:3)],col="red",pch=20,cex=2)
text(plsca.res$TExPosition.Data$fi[fi.2and2,c(2:3)],rownames(plsca.res$TExPosition.Data$fi[fi.2and2,c(2:3)]),col="red",pch=20,pos=3)
